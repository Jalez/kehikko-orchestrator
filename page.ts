 ...
 * ## Why it is generated
 *
 * The write ticket. It is minted once per process and has to reach the page
 * without being fetchable on a door of its own — a `GET /api/ticket` would be
 * exactly the endpoint the ticket exists to make unnecessary, readable by
 * anything that can reach the port. Printed into the document, it is reachable
 * by whatever can read the document, which under `storage: true` and no CORS is
 * this page and nothing in another tab. See `TICKET` in `doors.ts`.
 *
...
<html lang="en">
  <head>
    ...
    <script id="orchestrator-ticket" type="application/json">${JSON.stringify(ticket)}</script>
  </head>
  <body>
    <div id="root"></div>
