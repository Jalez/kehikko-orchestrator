 ...
 * this" is the one question it cannot afford to be confused about.
 *
 * ## What used to be underneath this
 *
 * `wire/host.ts` and `wire/mailbox.ts` — 404 lines — are one import now, and
 * three things went with them.
 *
 * The box below `connect` that caught an arrival which came too early and
 * replayed it once the assignment was done: the client splits `connect` from
 * `listen()`, so the ordering is three plain lines.
 *
 ...
 *
 * And the greeting's second argument, which the handler below had dropped at
 * its signature — see `onHello`.
 *
 * The `goto` backstop is passed explicitly as 900ms, because that is THIS
 * module's number and the client's default is 500. The essay for it stood in
 * `host.ts` and stands with the option now: the host gives a walk 1200ms and
 * then treats silence as "not found", and 900 leaves this page room for a
 * loopback fetch of its own roster while still beating that clock with a
...
export function useRoadmap(id: string): Roadmap {
  ...
  useEffect(() => {
     ...
     * the whole of a bug that made another module in this workspace hang
     * forever.
     *
     ...
     * mailbox exists — so `onHello` fires on that line. Anything that reads
     * `host.current` before the assignment finds null and returns early, and
     * the page sits on a sentence that never changes. It starts no timer
     * either, so nothing ever times out: not a slow answer, not a refusal, just
     * a page that stopped.
     *
     ...
     * two calls now: build, store, listen.
     */
    const live = connect(
      id,
      {
         ...
         *
         * `state` is whatever the host is keeping for this module. This handler
         * used to take one parameter, so the value arrived on the wire, was
         * parsed, was handed over — and stopped at this signature. It is named
         * and ignored now rather than absent: this page declares no `state:keep`
         * and holds nothing worth keeping, but a page that decides otherwise
         * should find the plumbing already here instead of rediscovering that
         * the host had been sending it all along.
         */
      ...
      /* 900ms, this module's own number, not the client's 500. See the essay at
         the top: the host gives a walk 1200ms, and 900 leaves room for a
         loopback fetch of the roster while still beating that clock. */
      { gotoBackstop: 900 },
    )
    host.current = live
    live.listen()
